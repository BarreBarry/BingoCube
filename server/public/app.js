(async () => {
  ...
  const API_BASE = '/api';                                    // same-origin API served by server.js
  let hostPassword = sessionStorage.getItem('rubiksHostPw') || '';  // sent with writes; the server verifies it
  async function loadGameRaw() {
    try {
      const r = await fetch(API_BASE + '/game', { cache: 'no-store' });
      if (!r.ok) return null;
  ...
  async function saveGameRaw(json) {
    try {
      const r = await fetch(API_BASE + '/game', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Host-Password': hostPassword },
        body: json
      });
      if (!r.ok) console.warn('save rejected by server:', r.status);
  ...
  function saveGame() {
    try { const j = JSON.stringify(game); lastGameJSON = j; saveGameRaw(j); }
  ...
  // Host auth is verified by the server (set the HOST_PASSWORD env var there).
  ...
  hostBtn.addEventListener('click', async () => {
    ...
    tapCount = 0;
    if (hostMode) { setHostMode(false); hostPassword = ''; sessionStorage.removeItem('rubiksHostPw'); return; }
    const pw = prompt('Enter host password:');
    if (pw === null) return;
    try {
      const r = await fetch(API_BASE + '/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: pw }) });
      if (r.ok) { hostPassword = pw; sessionStorage.setItem('rubiksHostPw', pw); setHostMode(true); }
